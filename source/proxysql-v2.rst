...
Installing ProxySQL 2.1.x on Debian or Ubuntu: ::

      $ sudo apt-get install proxysql21

Installing ProxySQL 2.1.x on Red Hat Enterprise Linux or CentOS: ::

      $ sudo yum install proxysql21
...
The ``proxysql2.0.x`` package or the ``proxysql2.1.x`` package from |percona| includes the ``proxysql-admin`` tool for
